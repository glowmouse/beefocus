...
{  
  ...
  hardware->DigitalWrite( HWI::Pin::MOTOR_ENA, HWI::low );        
...
unsigned int FOCUSER_STATE::state_set_dir()
{
  ...
  if ( state.arg0 ) {
    dir = true;
    hardware->DigitalWrite( HWI::Pin::DIR, HWI::high );        
  }
  else {
    dir = false;
    hardware->DigitalWrite( HWI::Pin::DIR, HWI::low );        
  }    
...
unsigned int FOCUSER_STATE::state_step_low_and_wait()
{
  ...
  hardware->DigitalWrite( HWI::Pin::STEP, HWI::low );
  state_stack.pop_back();
  return delay;
}
...
{
  int delay = 1000000 / focuser_speed / 2;
  hardware->DigitalWrite( HWI::Pin::STEP, HWI::high );
  state_stack.pop_back();
  return delay;
}
...
unsigned int FOCUSER_STATE::state_stop_at_home()
{
  ...
  if ( hardware->DigitalRead( HWI::Pin::HOME ) == 0 ) {
...
{
  hardware->DigitalWrite( HWI::Pin::MOTOR_ENA, HWI::high );        
  bool dont_accept_only_abort = false;
  check_for_commands( dont_accept_only_abort );
  return 100*1000;
}
